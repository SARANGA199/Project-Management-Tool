    ...
    const update = await RequestSV.findByIdAndUpdate(rId, updateReq)
        .then(() => {
            ...
            let mailOptions = {
                ...
                to: `${groupLeaderEmail}`,    //"savidyasamaranayake@gmail.com",
                subject: `Resquest Status ${supervisorStatus}  By Supervisor`,
...
export const updateCoSupervisor = async (req, res) => {
    const rId = req.params.id;

    const coSupervisor = req.body.resCosupervisors;

    const updateReqSupervisor = {
        rId,
        coSupervisor
    };

    const update = await RequestSV.findByIdAndUpdate(rId, updateReqSupervisor)
        .then(() => {
            res.status(200).send({ status: "Co-supervisor is Updated" });
        })
        .catch((err) => {
            console.log(err);
            res
